    ...
    class Node {
        ...
        Vector3D getChildCenter(int octant) {
            ...
            if ((octant & 4) == 0)
                x = center.x - halfWidth / 2;
            if ((octant & 2) != 0)
                y = center.y + halfWidth / 2;
            if ((octant & 2) == 0)
                y = center.y - halfWidth / 2;
            if ((octant & 1) != 0)
                z = center.z + halfWidth / 2;
            if ((octant & 1) == 0)
                z = center.z - halfWidth / 2;
        ...
        void updateDistributionMass(Body body) {
            ...
            if (centerOfMass == null)
                ...
                centerOfMass = centerOfMass.scale(prevMass).add(body.position.scale(body.mass)).scale(1.0 / totalMass);
        }

        int getOctant(Vector3D center, Vector3D bodyPos) {
            int index = 0;
            if (bodyPos.x >= center.x)
                index |= 4;
            if (bodyPos.y >= center.y)
                index |= 2;
            if (bodyPos.z >= center.z)
                index |= 1;
            return index;
        }
    }
